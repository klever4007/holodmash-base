...
# ==================== ВКЛАДКА 2: ПОИСК ПО НАПРАВЛЕНИЯМ ====================
with tab2:
    ...
    for i, cat in enumerate(CATEGORIES):
        with sub_tabs[i]:
            ...
            if records:
                ...
                for rec in records:
                    # Распаковываем кортеж строго по переменным
                    ...
                    with st.expander(title_text):
                        c1, c2 = st.columns(2)
                        with c1:
                            # Здесь используются переменные, а не функции ввода, конфликта больше нет!
...
# Закрываем соединение в самом конце скрипта
conn.close()
