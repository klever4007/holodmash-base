    ...
    for i, cat in enumerate(CATEGORIES):
        with sub_tabs[i]:
            ...
            if records:
                ...
                for rec in records:
                    # Корректно распаковываем кортеж с данными, чтобы не было путаницы
                    c_date, c_fio, c_city, c_company, c_email, c_need, c_notes, c_image = rec
                    
                    # Передаем в expander красивую понятную строку вместо сырого кортежа rec
...
# Закрываем соединение в самом конце работы приложения
conn.close()
import os
import base64
import psycopg2
