            ...
            if records:
                ...
                for rec in records:
                    with st.expander(f"📋 {rec[1]} | {rec[2]} ({rec[3]})"):
                        ...
                        with c2:
                            if isinstance(rec[7], str) and rec[7].startswith("data:image"):
                ...
                for rec in records:
                    with st.expander(f"📋 {rec} | {rec} ({rec})"):
                        ...
                        with c1:
                            ...
                            st.markdown(f"**📝 Рабочие заметки:**\n{rec}")
                        with c2:
                            # НАДЕЖНАЯ ПРОВЕРКА: Проверяем, что в rec лежит именно текст и он начинается как картинка
                ...
                for rec in records:
                    with st.expander(f"📋 {rec} | {rec} ({rec})"):
                        ...
                        with c1:
                            ...
                            st.markdown(f"**📝 Рабочие заметки:**\n{rec}")
                        with c2:
                            # ЗАЩИТА: Если ссылка битая или ведет на старый локальный ПК, сайт не упадет
                            if rec and rec != "-" and not rec.startswith("Папка photos"):
                                st.markdown("**📸 Скриншот переписки:**")
                                try:
                                    st.image(rec, use_container_width=True)
                                except Exception:
                                    st.caption("⚠️ Ошибка отображения старого скриншота")
                            else:
                                st.caption("ℹ️ Скриншот отсутствует")
            else:
                st.info(f"В направлении '{cat}' пока нет записей.")
...
def init_db():
    ...
    """)
    # 2. УМНАЯ ПРОВЕРКА: добавляем колонку category в старую базу, если её там не хватало
    try:
        cursor.execute("ALTER TABLE contacts ADD COLUMN category TEXT")
    except sqlite3.OperationalError:
...
def init_db():
    conn = sqlite3.connect(DB_FILE, check_same_thread=False)
    cursor = conn.cursor()
    # Добавили колонку category для направлений
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS contacts (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            fio TEXT, city TEXT, company TEXT, email TEXT, need TEXT, notes TEXT, image_url TEXT, created_at TEXT, category TEXT
...
with tab2:
    ...
    # Вкладки внутри поиска для разделения по направлениям
    sub_tabs = st.tabs([f"📦 {cat}" for cat in CATEGORIES])
        ...
        with sub_tabs[i]:
            if search:
                cursor.execute("""
                    SELECT created_at, fio, city, company, email, need, notes, image_url 
                    FROM contacts 
                    WHERE category = ? AND (fio LIKE ? OR city LIKE ? OR company LIKE ?)
                    ORDER BY id DESC LIMIT 1000
                """, (cat, f'%{search}%', f'%{search}%', f'%{search}%'))
            else:
                cursor.execute("""
                    SELECT created_at, fio, city, company, email, need, notes, image_url 
                    FROM contacts 
            ...
            # Выгрузка в Excel конкретного направления
            if records:
                import pandas as pd
                df = pd.DataFrame(records, columns=["Дата", "ФИО", "Город", "Компания", "Email", "Потребность", "Заметки", "Картинка"])
                df_excel = df.drop(columns=["Картинка"])
